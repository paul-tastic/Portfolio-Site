...
if (array_key_exists("pressedSubmit", $_POST)) {
		if (mysqli_connect_error()) {
			die("Database connection error.");
		} 
	$status = "bid submitted";
	$query = "INSERT INTO bids (`request_id`, `contractor_id`, `price`, `day_offered`, `expires`, `status`, `notes`) 
		VALUES (
			'".$jobID."',	
			'".$contractorID."',	
			'".mysqli_real_escape_string($link, $_POST['price'])."', 
			'".mysqli_real_escape_string($link, $_POST['day_offered'])."', 
			'".mysqli_real_escape_string($link, $_POST['expires'])."',
			'".$status."',
			'".mysqli_real_escape_string($link, $_POST['notes'])."')";
			if (!mysqli_query($link, $query)) {
				$error = "<p>Could not submit bid - Please try again.</p>";
			}
		...
		<div class="col-md-6" id="image-side">
			...
			<div class="col-md-6">
				<div class="col-md-12" id="exit-option-row">
					<button onclick="window.location.href='searchjobrequests.php'" type="button" class="btn btn-outline-warning choices">
  						Return to Search page
					</button>
					<button onclick="window.location.href='contractorpage.php'" type="button" class="btn btn-outline-primary choices">
  						Return to Dashboard page
					</button>
					<button type="submit" class="btn btn-success choices" form="bidForm" name="pressedSubmit">Submit Bid</button>
				</div>
	...
	<div class="col-md-12" id="bid-pane">
			...
			<form class="row" id="bidForm" method="POST">
			    ...
			    <div class="form-group col-md-2 mb-3">
					...
					<input type="text" class="form-control" name="day_offered" id="serviceDate" class="datepicker">
				</div>
				<div class="form-group col-md-2 mb-3">
			    	<label for="expireDate">Bid Expires: </label>
					<input type="text" class="form-control" name="expires" id="expireDate" class="datepicker">
				</div>
				<div class="form-group col-md-5 mb-3">
				    <label for="bidnotes">Remarks:</label>
				   	<textarea type="textarea" rows="2" wrap="hard" name="notes" class="form-control" id="inputNotes"></textarea>
				</div>
